...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    var newwidthpercent = sizeSwitcher(size);

    for (var i = 0; i < pizzacontainlen ; i++) {      
      pizzacontain[i].style.width = newwidthpercent + '%';
    }
  }
  var pizzacontain = [];
  pizzacontain = document.getElementsByClassName("randomPizzaContainer");
  var pizzacontainlen = pizzacontain.length;
...
// Utilizes the requestAnimationFrames functon to call the updatePositions function that moves the sliding background pizzas.


function animationtoupdatePositions() {
  if(!animating) {
    animating = true;
...
// Moves the sliding background pizzas based on scroll position. This function was optimized to move constant calculatons and DOM array logic outside the
//for loop.
...
function updatePositions() {
  ...
  for (var i = 0; i < 5; i++) {
      constArray.push(Math.sin((scrollpos / 1250) + i));
    }
  var itemleng = items.length;
  for (var i = 0; i < itemleng; i++) {
...
window.addEventListener('scroll', animationtoupdatePositions);
